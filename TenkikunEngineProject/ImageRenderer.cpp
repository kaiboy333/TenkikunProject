...
void ImageRenderer::Draw()
{
	if (image != nullptr) {
		if (image->GetGH() != -1) {
			...
			DrawRotaGraph3F(drawPos.x, drawPos.y, image->GetWidth() / 2, image->GetHeight() / 2, scale.x * zoom, scale.y * zoom, -MyMath::EulerToRad(rota.z), image->GetGH(), true, isFlipX, isFlipY);	//•`‰æ
		}
	}
}